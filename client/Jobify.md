...
- User.js
- setup schema
- name(trim:true), email, password, lastName (trim:true), location
- lastName and location not required
- all {type:String}
...
- setup app.use(express.json()) in the server.js
...
const errorHandlerMiddleware = (err, req, res, next) => {
  ...
  if (err.name === "ValidationError") {
    ...
    defaultError.msg = Object.values(err.errors) // iterate over array
...
- if one property from req.body is missing, throw new BadRequestError

#### Check for duplicate email in controller

- create userAlreadyExists that awaits for User.findOne and pass the email
- if user already exists, throw bad request error
